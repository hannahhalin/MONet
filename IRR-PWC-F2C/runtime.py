...
class EvaluationEpoch:
    ...
    def save_outputs(self, example_dict, output_dict):
        ...
        # Hard-coded for convinience for now.
        b_size = output_dict["occ_2"].data.size(0)
        ...
        if self._args.save_result_occ:
            if self._args.save_result_bidirection:
                ...
                # Draw the opposite label.
                output_occs, output_occ_scores, gt_occs = [], [], []
                raw_output_occs = []
                if 'occ_7' in output_dict.keys():
                    start_idx = 7
                else:
                    start_idx = 6
                for i in range(start_idx, -1, -1):
                    occ_key = 'occ_' + str(i)
                    output_occ = np.round(
                        1 - nn.Sigmoid()(output_dict[occ_key]).expand(-1, 3, -1, -1).data.cpu().numpy().transpose(
                            [0, 2, 3, 1])) * 255
                    output_occ_score = (1 - nn.Sigmoid()(output_dict[occ_key]).expand(-1, 3, -1, -1).data.cpu().numpy().transpose([0, 2, 3, 1])) * 255
                    gt_occ = (1 - example_dict["target_occ1"].expand(-1, 3, -1, -1).data.cpu().numpy().transpose([0, 2, 3, 1])) * 255

                    raw_output_occ = nn.Sigmoid()(output_dict[occ_key]).data.cpu().numpy().transpose([0, 2, 3, 1])
                    
                    output_occs.append(output_occ)
                    output_occ_scores.append(output_occ_score)
                    gt_occs.append(gt_occ)
                    raw_output_occs.append(raw_output_occ)
        ...
        if self._args.save_result_img:
            for ii in range(0, b_size):
                if self._args.save_result_occ:
                    idx = 0
                    for output_occ, output_occ_score, gt_occ, raw_output_occ in zip(output_occs, output_occ_scores, gt_occs, raw_output_occs):
                        file_name_occ = file_names_img[ii] + '_occ_pred_scale-' + str(7-idx) + '.png'
                        imageio.imsave(file_name_occ, output_occ[ii].astype(np.uint8))

                        file_name_occ_score = file_names_img[ii] + '_occ_score_scale-' + str(7-idx) + '.png'
                        imageio.imsave(file_name_occ_score, output_occ_score[ii].astype(np.uint8))

                        file_name_occ = file_names_img[ii] + '_occ_gt_scale-' + str(7-idx) + '.png'
                        imageio.imsave(file_name_occ, gt_occ[ii].astype(np.uint8))

                        file_name_raw_occ = file_names_img[ii] + '_raw_occ_scale-' + str(7-idx) + '.npy'
                        np.save(file_name_raw_occ, raw_output_occ[ii])

                        if self._args.save_result_bidirection:
                            imageio.imsave(file_names_img[ii] + '_occ_b.png', output_occ_b[ii].astype(np.uint8))
