...
def upsample2d_as(inputs, target_as, mode="bilinear"):
    ...
    # tf.interpolate: Down/up samples the input to either the given size or the given scale_factor
...
def concat_and_sync_dims(inputs, size_idx=0):
    '''
        Assume: The first item in the list has the largest dimensions.
    '''
    outputs = []
    for item in inputs:
        outputs.append(upsample2d_as(item, inputs[size_idx], mode='bilinear'))
