...
class MultiScaleEPE_PWC_Bi_Occ_upsample(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        ...
        if self.training:
            ...
            for output_type in output_dict.keys():
                if 'flow' in output_type:
                    loss_dict[output_type + "_epe"] = _elementwise_epe(output_dict["flow"], target_dict["target1"]).mean()
                elif 'occ' in output_type:
                    # Add performance of multi-scale outputs.
                    loss_dict[output_type + "_F1"] = f1_score(
                        target_dict["target_occ1"], torch.round(self.occ_activ(output_dict[output_type])))
                else:
...
class MultiScaleEPE_PWC_upsample_No_Occ(nn.Module):
    def __init__(self, args):

        super(MultiScaleEPE_PWC_upsample_No_Occ, self).__init__()
        self._args = args
        self._batch_size = args.batch_size
    ...
    def forward(self, output_dict, target_dict):
        ...
        if self.training:
            ...
            # Loss for occlusion is 0.
            loss_dict["flow_loss"] = flow_loss / self._batch_size
            # Tensorize it.
            occ_loss = torch.tensor(occ_loss)
            loss_dict["occ_loss"] = occ_loss / self._batch_size
...
class MultiScale_PWC_Bi_Occ_upsample_No_Flow(nn.Module):
    def __init__(self,
                 args):

        super(MultiScale_PWC_Bi_Occ_upsample_No_Flow, self).__init__()
        self._args = args
        self._batch_size = args.batch_size
    ...
    def forward(self, output_dict, target_dict):
        ...
        if self.training:
            ...
            loss_dict["flow_loss"] = torch.tensor(0.0)
            loss_dict["occ_loss"] = occ_loss / self._batch_size
            loss_dict["total_loss"] = occ_loss / self._batch_size

        else:
            for output_type in output_dict.keys():
                if 'flow' in output_type:
                    loss_dict["epe"] = torch.tensor(0.0)
                elif 'occ' in output_type:
                    # Add performance of multi-scale outputs.
                    loss_dict[output_type + "_F1"] = f1_score(
                        target_dict["target_occ1"], torch.round(self.occ_activ(output_dict[output_type])))
