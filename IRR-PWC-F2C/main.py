...
def main():
    ...
    # Whether freeze the model.
    if not args.freeze_weight_name == '':
        print('\n\nFreeze the weights containing name:', args.freeze_weight_name)
        for name, p in model_and_loss.named_parameters():
            if args.freeze_weight_name in name:
                print('Freezing weight:', name)
